...
$sbi_download_name = "Instagram Feed Pro Developer";
...
if (function_exists('sb_instagram_feed_init') || function_exists('display_instagram')) {
	...
	/**
	 * Initialize the Instagram Feed Pro plugin.
	 *
	 * Includes necessary files and initializes comment listeners.
	 */
	...
	add_action('init', 'sb_instagram_feed_pro_init', 11);
	...
	function sbi_cron_custom_interval($schedules)
	{
		$schedules['sbi30mins'] = array(
			'interval' => 30 * 60,
			'display'  => __('Every 30 minutes', 'instagram-feed')
		);
		$schedules['sbiweekly'] = array(
			'interval' => 3600 * 24 * 7,
			'display'  => __('Weekly', 'instagram-feed')
		);
	 ...
	 *
	 * @param  bool $network_wide If the plugin is being activated network-wide.
	 *
	...
	function sb_instagram_uninstall()
	{
		...
		$image_files = glob(trailingslashit($upload['basedir']) . trailingslashit('sb-instagram-feed-images') . '*');
	...
	/**
	 * Initializes the plugin updater for the Instagram Feed Pro plugin.
	 *
	 * This function retrieves the license key from the database and sets up the
	 * Easy Digital Downloads Software Licensing (EDD SL) plugin updater with the
	 * necessary information such as the current version number, license key, plugin
	 * name, and author.
	 *
	 * @return void
	 */

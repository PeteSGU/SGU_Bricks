...
function tnc_pvfw_site_registered_message() {
	...
	if( ! $get_reg_status ){
		tnc_pvfw_site_registered_status(true);
	}
...
if ( ! function_exists( 'themencode_advertisement_update' ) ) {
   ...
   // add_action( 'admin_notices', 'themencode_advertisement_update' ); // disabling ad display for now
}
...
function tnc_pvfw_table_url_cell_content( $colname ) {
	if ( $colname == 'tnc_pvfw_post_header_for_display_url') {
		...
		if( isset( $get_pvfw_single_settings['default-zoom'] ) && !empty( $get_pvfw_single_settings['default-zoom'] ) ){
			...
			if( $zoom == "global" ){
				$zoom = $get_pvfw_global_settings['toolbar-default-zoom'];
			}
		} else {
			$zoom = 'page-fit';
		}
		...
		if( isset( $get_pvfw_single_settings['toolbar-default-page-mode'] ) && !empty( $get_pvfw_single_settings['toolbar-default-page-mode'] ) ){
			...
			if( $pagemode == "global" ){
				$pagemode = $get_pvfw_global_settings['toolbar-default-page-mode'];
			}
		} else {
			$pagemode = 'none';
		}
		...
		<?php
			$input_id = 'pvfw_tnc_copy_link_url_' . intval( $url_id );
			?>

			<input type="text"
				id="<?php echo esc_attr( $input_id ); ?>"
				class="pvfw_tnc_copy_link_url attachment-details-copy-link tnc-pvfw-copy-share-url"
				value="<?php echo esc_url( $full_url ); ?>"
				readonly>

			<span class="copy-to-clipboard-container">
				<button class="button button-small pvfw-copy-btn"
					type="button"
					data-clipboard-target="#<?php echo esc_attr( $input_id ); ?>"
					data-message-id="copied-message-<?php echo esc_attr( $input_id ); ?>">
					<?php esc_html_e( 'Copy URL to the clipboard', 'pdf-viewer-for-wordpress' ); ?>
				</button>
			</span>

			
				<!-- Div to show "Copied!" message -->
				<div id="copied-message-<?php echo esc_attr( $input_id ); ?>" class="pvfw-copied-message">
					<?php esc_html_e( 'URL copied to clipboard', 'pdf-viewer-for-wordpress' ); ?>
				</div>
...
function tnc_pvfw_auto_add_hash_params_to_viewer_url(){
	...
	if( is_singular( 'pdfviewer' ) && $get_post_status == 'publish' ){
		...
		if( $oxygen_redirect == '1' ){
			...
			if( isset( $get_pvfw_single_settings['default_scroll'] ) && !empty( $get_pvfw_single_settings['default_scroll'] ) ){
				$default_scroll = $get_pvfw_single_settings['default_scroll'];
				if( $default_scroll == "global" ){
					$default_scroll = $get_pvfw_global_settings['toolbar-default-scroll'];
				}
			} else {
				$default_scroll = '0';
			}

			if( isset( $get_pvfw_single_settings['default_spread'] ) && !empty( $get_pvfw_single_settings['default_spread'] ) ){
				$default_spread = $get_pvfw_single_settings['default_spread'];
				if( $default_spread == "global" ){
					$default_spread = $get_pvfw_global_settings['toolbar-default-spread'];
				}
			} else {
				$default_spread = '0';
			}

			$generated_url  = 'file=' . $get_pvfw_single_settings['file'] . '&settings=' . $download . $print . $zoom  . $fullscreen . $share . $open . $logo . $pagenav . $find  . $current_view . $rotate . $handtool . $doc_prop . $toggle_menu . $toggle_left . $scroll . $spread . $default_scroll . $default_spread . '&lang=' . $get_pvfw_single_settings['language'];

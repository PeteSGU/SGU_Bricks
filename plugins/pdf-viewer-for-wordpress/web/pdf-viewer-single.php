...
if ( ! defined( 'THEMENCODE_PDF_VIEWER_SINGLE' ) ) {
...
// Use Global option settings.
$toolbar_use_global     = $get_pvfw_single_settings['toolbar-elements-use-global-settings'];
$appearance_use_global  = $get_pvfw_single_settings['appearance-use-global-settings'];

if( isset( $get_pvfw_global_settings['general-bg-image-settings'] ) ) {
	$general_bg_image_settings = $get_pvfw_global_settings['general-bg-image-settings'];
} else {
	$general_bg_image_settings = 0;
}

if( isset( $get_pvfw_single_settings['default-viewer-bg-image-settings'] ) ) {
	$default_viewer_bg_image_settings = $get_pvfw_single_settings['default-viewer-bg-image-settings'];
} else {
	$default_viewer_bg_image_settings = 0;
}

// retrieve options.

if($appearance_use_global == '0'){
	if(isset($get_pvfw_single_settings['appearance-disable-flip-sound'])){
		$pvfw_flip_audio  = $get_pvfw_single_settings['appearance-disable-flip-sound'];
	}else{
		$pvfw_flip_audio = 0;
	}
}else{
	if(isset($get_pvfw_global_settings['appearance-disable-flip-sound'])){
...
if ( $toolbar_use_global == '0' ) {
	...
	$find         = ( $get_pvfw_global_settings['toolbar-find'] == '0' ) ? 'off' : 'on';
	$pagenav      = ( $get_pvfw_global_settings['toolbar-pagenav'] == '0' ) ? 'off' : 'on';
	$share        = ( $get_pvfw_global_settings['toolbar-share'] == '0' ) ? 'off' : 'on';
	$zoom         = ( $get_pvfw_global_settings['toolbar-zoom'] == '0' ) ? 'off' : 'on';
	$logo         = ( $get_pvfw_global_settings['toolbar-logo'] == '0' ) ? 'off' : 'on';
	$print        = ( $get_pvfw_global_settings['toolbar-print'] == '0' ) ? 'off' : 'on';
	$open         = ( $get_pvfw_global_settings['toolbar-open'] == '0' ) ? 'off' : 'on';
	$download     = ( $get_pvfw_global_settings['toolbar-download'] == '0' ) ? 'off' : 'on';
	$fullscreen   = ( $get_pvfw_global_settings['toolbar-fullscreen'] == '0' ) ? 'off' : 'on';
	$current_view = ( $get_pvfw_global_settings['toolbar-current-view'] == '0' ) ? 'off' : 'on';
	$rotate       = ( $get_pvfw_global_settings['toolbar-rotate'] == '0' ) ? 'off' : 'on';
	$handtool     = ( $get_pvfw_global_settings['toolbar-handtool'] == '0' ) ? 'off' : 'on';
	$doc_prop     = ( $get_pvfw_global_settings['toolbar-doc-prop'] == '0' ) ? 'off' : 'on';
	$toggle_menu  = ( $get_pvfw_global_settings['toolbar-right-toggle'] == '0' ) ? 'off' : 'on';
	$toggle_left  = ( $get_pvfw_global_settings['toolbar-left-toggle'] == '0' ) ? 'off' : 'on';
	$scroll       = ( $get_pvfw_global_settings['toolbar-scroll'] == '0' ) ? 'off' : 'on';
	$spread       = ( $get_pvfw_global_settings['toolbar-spread'] == '0' ) ? 'off' : 'on';
}
...
if ( $appearance_use_global == '0' ) {
	...
	$tnc_primary_color   = $get_pvfw_single_settings['appearance-select-colors']['primary-color'];
	$tnc_secondary_color = $get_pvfw_single_settings['appearance-select-colors']['secondary-color'];
	$tnc_text_color      = $get_pvfw_single_settings['appearance-select-colors']['text-color'];
	$tnc_icon_color      = $get_pvfw_single_settings['appearance-select-icon'];
} else {
	...
	if( isset( $get_pvfw_global_settings['appearance-select-colors']['primary-color'] ) ) {
		$tnc_primary_color   = $get_pvfw_global_settings['appearance-select-colors']['primary-color'];
	} else {
		$tnc_primary_color   = '';
	}
	
	if( isset( $get_pvfw_global_settings['appearance-select-colors']['secondary-color'] ) ) {
		$tnc_secondary_color = $get_pvfw_global_settings['appearance-select-colors']['secondary-color'];
	} else {
		$tnc_secondary_color = '';
	}

	if( isset( $get_pvfw_global_settings['appearance-select-colors']['text-color'] ) ) {
...
if( $appearance_use_global == 0 ) {
	if( isset( $get_pvfw_single_settings['default-logo']['url'] ) ) {
		$logo_image_url         = $get_pvfw_single_settings['default-logo']['url'];
	} else {
		$logo_image_url         = '';
	}	
	if( isset( $get_pvfw_single_settings['default-favicon']['url'] ) ) {
		$favicon_url            = $get_pvfw_single_settings['default-favicon']['url'];
	} else {
		$favicon_url            = '';
	}

	if ( $default_viewer_bg_image_settings == '1') {
		if( isset( $get_pvfw_single_settings['default-bg-img']['url'] ) ) {
			$bg_img_url            = $get_pvfw_single_settings['default-bg-img']['url'];
		} else {
			$bg_img_url           = '';
		}
		if( isset( $get_pvfw_single_settings['default-bg-img-size'] ) ) {
			$bg_img_size            = $get_pvfw_single_settings['default-bg-img-size'];
		} else {
			$bg_img_size           = 'cover';
		}
		if( isset( $get_pvfw_single_settings['default-bg-img-repeat'] ) ) {
			$bg_img_repeat            = $get_pvfw_single_settings['default-bg-img-repeat'];
		} else {
			$bg_img_repeat           = 'no-repeat';
		}
		if( isset( $get_pvfw_single_settings['default-bg-img-attachment'] ) ) {
			$bg_img_attachment            = $get_pvfw_single_settings['default-bg-img-attachment'];
		} else {
			$bg_img_attachment           = 'scroll';
		}
		if( isset( $get_pvfw_single_settings['default-bg-img-position'] ) ) {
			$bg_img_position            = $get_pvfw_single_settings['default-bg-img-position'];
		} else {
	...
	if( isset( $get_pvfw_global_settings['general-logo']['url'] ) ) {
		$logo_image_url         = $get_pvfw_global_settings['general-logo']['url'];
	} else {
		$logo_image_url         = '';
	}	
	if( isset( $get_pvfw_global_settings['general-favicon']['url'] ) ) {
	...
	if( $general_bg_image_settings == '1') {
		if( isset( $get_pvfw_global_settings['general-bg-img']['url'] ) ) {
			$bg_img_url            = $get_pvfw_global_settings['general-bg-img']['url'];
		} else {
			$bg_img_url           = '';
		}
		if( isset( $get_pvfw_global_settings['general-bg-img-size'] ) ) {
			$bg_img_size            = $get_pvfw_global_settings['general-bg-img-size'];
		} else {
			$bg_img_size           = 'cover';
		}
		if( isset( $get_pvfw_global_settings['general-bg-img-repeat'] ) ) {
			$bg_img_repeat            = $get_pvfw_global_settings['general-bg-img-repeat'];
		} else {
			$bg_img_repeat           = 'no-repeat';
		}
		if( isset( $get_pvfw_global_settings['general-bg-img-attachment'] ) ) {
			$bg_img_attachment            = $get_pvfw_global_settings['general-bg-img-attachment'];
		} else {
			$bg_img_attachment           = 'scroll';
		}
		if( isset( $get_pvfw_global_settings['general-bg-img-position'] ) ) {
			$bg_img_position            = $get_pvfw_global_settings['general-bg-img-position'];
		} else {
			$bg_img_position           = 'center center';
		}
	}
...
if( array_key_exists('default-return-text', $get_pvfw_single_settings ) ){
	$get_return_link_text   = $get_pvfw_single_settings['default-return-text'];
} else {
	$get_return_link_text   = esc_html__( 'Return to Site', 'pdf-viewer-for-wordpress' );
}
...
/**
 * Display or Hide Share
 *
 * @param string $p_share share setting.
 * @return void
 */
...
if ( function_exists( 'wfam_has_access' ) ) {
	...
	if ( ! wfam_has_access( $file_requested ) ) {
		...
		if ( empty( $get_wfam_heading ) ) {
			$get_wfam_heading = esc_html__( 'SORRY', 'pdf-viewer-for-wordpress' );
		}

		if ( empty( $get_wfam_content ) ) {
			$get_wfam_content = esc_html__( 'You do not have permission to view this file, please contact us if you think this was by a mistake.', 'pdf-viewer-for-wordpress' );
		}

		if ( empty( $get_wfam_btn_text ) ) {
			$get_wfam_btn_text = esc_html__( 'Go To Homepage', 'pdf-viewer-for-wordpress' );
		}
...
<head>
	...
	<meta property="og:image" content="
		...
		if ( has_post_thumbnail( get_the_ID() ) ) {
			...
			echo apply_filters( 'tnc_pvfw_facebook_share_thumb_url', plugins_url() . '/' . PVFW_PLUGIN_DIR . '/images/thumb.png' ); }
		?>
	">
...
<body <?php body_class(); ?> >
	<div class='pvfw-not-allowed'>
		...
		<p><?php echo do_shortcode( $get_wfam_content ); ?></p>
		<a class='tnc-btn tnc-go-home-btn' target="_parent" href='<?php echo $get_wfam_btn_url; ?>'><?php echo $get_wfam_btn_text; ?></a>
	</div>
...
<?php
	...
	if( ( !empty( $bg_img_url ) && $default_viewer_bg_image_settings == '1' ) || ( !empty( $bg_img_url ) && $general_bg_image_settings == '1' ) ) {
		echo "style=' background-image: url( $bg_img_url ); background-size: $bg_img_size; background-repeat: $bg_img_repeat; background-attachment: $bg_img_attachment; background-position: $bg_img_position; '";
	}
?>
>
<div id="outerContainer">
	...
	<div class="toolbar">
		...
		<div id="toolbarViewer">
			...
			<?php
				$tnc_pdf_voice_default_speed =  isset($get_pvfw_single_settings['default-speed']) ? $get_pvfw_single_settings['default-speed'] : '';
				$tnc_pdf_voice_default_pitch =  isset($get_pvfw_single_settings['default-pitch']) ? $get_pvfw_single_settings['default-pitch'] : '';
			    $visisbility_voice_card 	 =  isset($get_pvfw_single_settings['toolbar-elements-use-transcribe']) ? $get_pvfw_single_settings['toolbar-elements-use-transcribe']  : 0;
				$visibility_voice 			 =  isset($get_pvfw_single_settings['visibility-voice']) ? $get_pvfw_single_settings['visibility-voice'] : '';
				$visibility_speed 			 =  isset($get_pvfw_single_settings['visibility-speed']) ? $get_pvfw_single_settings['visibility-speed'] : '';
				$visibility_pitch 			 =  isset($get_pvfw_single_settings['visibility-pitch']) ? $get_pvfw_single_settings['visibility-pitch'] : '';

				// function tnc_pvfw_voice_card ($visisbility_voice_card) {
				// 	return ($visisbility_voice_card == '0') ? 'display:none': '';
				// }
				function tnc_pvfw_voice($visibility_voice) {
					return ($visibility_voice == '0') ? 'display: none;' : '';
				}

			
				function tnc_pvfw_rate($visibility_speed) {
					return ($visibility_speed == '0') ? 'display: none;' : '';
				}

				
				function tnc_pvfw_pitch($visibility_pitch) {
					return ($visibility_pitch == '0') ? 'display: none;' : '';
				}

				if(  $visisbility_voice_card === '1' ) {
					?>
					<button id="tncHeadphone" class="toolbarButton"></button>
					<?php 
				}
				...
				<div class="tnc-synthesis-wrapper" style="opacity: 0; visibility: hidden; transition: opacity 0.5s ease;">
					<div class="tnc-synthesis-container">
					    <div class="tnc-synthesis-close-icon"> <span> &#x2715; </span> </div>
					  	<!-- Speed Control -->
						<div class="tnc_pdf_rate" style="<?php echo esc_attr(tnc_pvfw_rate($visibility_speed)); ?>">
							<label for="rate"><?php esc_html_e('Rate:', 'pdf-viewer-for-wordpress'); ?></label>
							<input type="range" id="rate" min="0.5" max="2" step="0.1" 
								value="<?php echo esc_attr($tnc_pdf_voice_default_speed); ?>">
							<span id="rateValue"><?php echo esc_html($tnc_pdf_voice_default_speed); ?></span>
						</div>
						
						<!-- Pitch Control -->
						<div class="tnc_pdf_pitch" style="<?php echo esc_attr(tnc_pvfw_pitch($visibility_pitch)); ?>">
							<label for="pitch"><?php esc_html_e('Pitch:', 'pdf-viewer-for-wordpress'); ?></label>
							<input type="range" id="pitch" min="0.5" max="2" step="0.1" 
								value="<?php echo esc_attr($tnc_pdf_voice_default_pitch); ?>">
							<span id="pitchValue"><?php echo esc_html($tnc_pdf_voice_default_pitch); ?></span>
						</div>
						
						<!-- Voice Selection -->
						<div class="tnc_pdf_voice" style="<?php echo esc_attr(tnc_pvfw_voice($visibility_voice)); ?>">
							<label for="voiceSelect"><?php esc_html_e('Voice:', 'pdf-viewer-for-wordpress'); ?></label>
							<select id="voiceSelect"></select>
						</div>
						
						<!-- Control Buttons -->
						<div class="tnc_voice_group_btn">
							<button id="speakButton" class="toolbarButton" title="<?php esc_html_e( 'Read Aloud', 'pdf-viewer-for-wordpress' )?> ">
								<?php esc_html_e('Read', 'pdf-viewer-for-wordpress'); ?>
							</button>
							<button id="pauseButton" class="toolbarButton" title="<?php esc_html_e( 'Pause', 'pdf-viewer-for-wordpress' )?> " disabled>
								<?php esc_html_e('Pause', 'pdf-viewer-for-wordpress'); ?>
							</button>
							<button id="resumeButton" class="toolbarButton" title="<?php esc_html_e( 'Resume', 'pdf-viewer-for-wordpress' )?> " disabled>
								<?php esc_html_e('Resume', 'pdf-viewer-for-wordpress'); ?>
							</button>
							<button id="stopButton" class="toolbarButton" title="<?php esc_html_e( 'Stop', 'pdf-viewer-for-wordpress' )?> " disabled>
								<?php esc_html_e('Stop', 'pdf-viewer-for-wordpress'); ?>
							</button>
						</div>
					</div>
				</div>
...
<input class="r-btn-style" type="reset" name="reset" value="Reset">
	...
	<div class="tnc-pdf-back-to-btn-wrapper">
		...
		?>
		<a class="tnc-pdf-back-to-btn" href="<?php echo esc_html( $return_link ); ?>">
		<?php
		if ( ! empty( $get_return_link_text ) ) {
			$return_link_text = $get_return_link_text;
		} else {
			$return_link_text = esc_html__( 'Return to Site', 'pdf-viewer-for-wordpress' );
		}

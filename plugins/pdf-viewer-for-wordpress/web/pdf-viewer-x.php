...
$toggle_left  = $settings_arr[14];
$scroll       = $settings_arr[15];
$spread       = $settings_arr[16];

var_dump($settings_arr[17]);

if ( isset( $settings_arr[17] ) ) {
	$default_scroll = $settings_arr[17];
} else {
	$default_scroll = '3';
}

if ( isset( $settings_arr[18] ) ) {
	$default_spread = $settings_arr[18];
} else {
	$default_spread = '0';
}
...
$flip_type =  ( isset( $get_pvfw_global_settings['flip_type'] ) ) ? $get_pvfw_global_settings['flip_type'] : '' ;
$page_turning_speed = ( isset( $get_pvfw_global_settings['page-turning-speed'] ) ) ? $get_pvfw_global_settings['page-turning-speed'] : '500';
...
if ( isset( $_GET['demo-theme'] ) && ! empty( $_GET['demo-theme'] ) ) {
	$style_theme = $_GET['demo-theme'] . '.css';
}
...
<head>
	...
	<link rel="icon" href="<?php esc_url( $favicon_url ); ?>">
...
<body <?php body_class(); ?> >
	<div class='pvfw-not-allowed'>
		...
		<a class='tnc-btn tnc-go-home-btn' target='_parent' href='<?php echo home_url(); ?>'><?php echo esc_html_e( "Go To Homepage", $domain = 'pdf-viewer-for-wordpress' ) ?></a>
	</div>
...
<head>
    ...
    <meta name="google" content="notranslate">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <link rel="shortcut icon" href="<?php echo esc_html__( $favicon_url, 'pdf-viewer-for-wordpress' ); ?>">
    <title><?php echo basename($file, ".pdf"); ?> - <?php bloginfo('name'); ?></title>
    <script type="text/javascript" src="<?php echo plugins_url()."/".TNC_PVFW_RESOURCES_DIR."/jquery.min.js?".PVFW_PLUGIN_VERSION; ?>"></script>
    <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/viewer.css?".PVFW_PLUGIN_VERSION; ?>"/>
    <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/schemes/common.css?".PVFW_PLUGIN_VERSION; ?>"/>
    <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/schemes/".$style_theme; ?>"/>
     <?php 
	 	//Add icon & toolbar style			
		 $get_pvfw_global_settings = get_option( 'pvfw_csf_options' );

		 if( array_key_exists( 'appearance-icon-size', $get_pvfw_global_settings ) ){
			 $get_pvfw_icon_size   = $get_pvfw_global_settings['appearance-icon-size'];
		 } else {
			 $get_pvfw_icon_size   = 'medium';
		 }

		 if( array_key_exists( 'appearance-select-toolbar-style', $get_pvfw_global_settings ) ){
		 ...
		 if( 'small' ==  $get_pvfw_icon_size) { ?>
			  <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/icon-size/small.css?".PVFW_PLUGIN_VERSION; ?>"/>
			<?php 
		 } elseif ( 'large' == $get_pvfw_icon_size) { ?>
			  <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/icon-size/large.css?".PVFW_PLUGIN_VERSION; ?>"/>
			<?php 
		 } else {  ?>
			  <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/icon-size/medium.css?".PVFW_PLUGIN_VERSION; ?>"/>
			<?php
		 }

		 //Load toolbar style only. 
		 if( 'bottom-center' ==  $get_pvfw_toolbar_style ){ ?>
			   <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/toolbar-style/bottom-center.css?".PVFW_PLUGIN_VERSION; ?>"/>
			<?php 
		 } elseif ('bottom-full-width' ==  $get_pvfw_toolbar_style ) { ?>
			  <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/toolbar-style/bottom-full-width.css?".PVFW_PLUGIN_VERSION; ?>"/>
			<?php
		 } elseif ('top-center' ==  $get_pvfw_toolbar_style ) { ?>
			  <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/toolbar-style/top-center.css?".PVFW_PLUGIN_VERSION; ?>"/>
			<?php
		 } else { ?>
			  <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_WEB_DIR."/toolbar-style/top-full-width.css?".PVFW_PLUGIN_VERSION; ?>"/>
			<?php
		 }

	 ?>
    <script type="text/javascript" src="<?php echo plugins_url().'/'.TNC_PVFW_WEB_DIR.'/'; ?>compatibility.js?<?php echo PVFW_PLUGIN_VERSION; ?>"></script>

    <link rel="resource" type="application/l10n" href="<?php echo plugins_url().'/'.TNC_PVFW_WEB_DIR.'/'; ?>locale/locale.properties" >
    <script type="text/javascript" src="<?php echo plugins_url().'/'.TNC_PVFW_BUILD_DIR.'/'; ?>pdf.js?<?php echo PVFW_PLUGIN_VERSION; ?>"></script>

    <script type="text/javascript" src="<?php echo plugins_url().'/'.TNC_PVFW_WEB_DIR.'/'; ?>debugger.js?<?php echo PVFW_PLUGIN_VERSION; ?>"></script>
    <script type="text/javascript" src="<?php echo plugins_url()."/".TNC_PVFW_RESOURCES_DIR."/pinch-zoom.js?". PVFW_PLUGIN_VERSION; ?>"></script>
    <script src="<?php echo plugins_url()."/".TNC_PVFW_RESOURCES_DIR."/jquery.modal.min.js?".PVFW_PLUGIN_VERSION; ?>"></script>
    <link rel="stylesheet" href="<?php echo plugins_url()."/".TNC_PVFW_RESOURCES_DIR."/jquery.modal.min.css?".PVFW_PLUGIN_VERSION; ?>" />
    ...
    <meta name="twitter:card" content="summary_large_image">
     ...
     <script type="text/javascript">
      var tnc_locale = '<?php echo esc_html( $viewer_language ); ?>';
      var tnc_imageResourcesPath = "<?php echo plugins_url().'/'.TNC_PVFW_WEB_DIR.'/'; ?>images/";
      var tnc_workerSrc = "<?php echo plugins_url().'/'.TNC_PVFW_BUILD_DIR.'/'; ?>pdf.worker.js?<?php echo PVFW_PLUGIN_VERSION; ?>";
      var tnc_sandboxSrc = "<?php echo plugins_url() . '/' . TNC_PVFW_BUILD_DIR; ?>/pdf.sandbox.js?<?php echo PVFW_PLUGIN_VERSION; ?>";
      var tnc_cMapUrl = "<?php echo plugins_url().'/'.TNC_PVFW_WEB_DIR.'/'; ?>cmaps/";
      var tnc_cMapPacked = true;

      var fto = "<?php echo esc_html( $encode_file ); ?>";
      var tnc_print = "<?php echo ($print == "on" ? 0 : 1); ?>";
      var tnc_dl = "<?php echo ($download == "on" ? 0 : 1); ?>";
	  var tnc_same_mobile_spread = "0";
	  var tnc_flip_type = "<?php echo esc_html( $flip_type ); ?>";
      var tnc_page_turning_speed = "<?php echo esc_html( $page_turning_speed ); ?>";
      var tnc_scroll_default = <?php echo esc_html( $default_scroll ); ?>;
      var tnc_spread_default = <?php echo esc_html( $default_spread ); ?>;
      var tnc_stdfonts = "<?php echo plugins_url().'/'.TNC_PVFW_WEB_DIR.'/'; ?>web/standard_fonts/";
     </script>
     <script type="text/javascript" src="<?php echo plugins_url().'/'.TNC_PVFW_WEB_DIR.'/'; ?>viewer.js?<?php echo PVFW_PLUGIN_VERSION; ?>"></script>
      
       <!-- $PVFW_FB: additional files included -->
    <script src="<?php echo plugins_url().'/'.TNC_PVFW_RESOURCES_DIR.'/'; ?>turn.min.js?<?php echo PVFW_PLUGIN_VERSION; ?>"></script>
    <link rel="stylesheet" href="<?php echo plugins_url().'/'.TNC_PVFW_WEB_DIR.'/'; ?>pdf-turn/pdf-turn.css?<?php echo PVFW_PLUGIN_VERSION; ?>">
    <script src="<?php echo plugins_url().'/'.TNC_PVFW_WEB_DIR.'/'; ?>pdf-turn/pdf-turn.js?<?php echo PVFW_PLUGIN_VERSION; ?>"></script>
    ...
    <style type="text/css">
		...
		.doorHanger:before, .doorHangerRight:before {
   			 border-bottom-color: var(--doorhanger-border-color) !important;
		 }
        .logo_text a img {
          max-width: 120px;
        }
...
<?php } ?>
<body class="loadingInProgress <?php 
   //get toolbar && Icon class
   $get_pvfw_global_settings = get_option( 'pvfw_csf_options' );
	...
	switch ($get_pvfw_toolbar_style) {
	case "bottom-center":
		echo "toolbar-bottom-center ";
		break;
	case "bottom-full-width":
		echo "toolbar-bottom-full-width ";
		break;
	case "top-center":
		echo "toolbar-top-center ";
		break;
	case "top-full-width":
		echo "toolbar-top-full-width ";
		break;
	default:
		echo "toolbar-top-full-width ";
	}

	switch ($get_pvfw_icon_size) {
	case "small":
		echo "icon-small ";
		break;
	case "medium":
...
<script type="text/javascript">
	  ...
	  <?php if ( $pagenav == '1' ) { ?>
		<button style="" class=" pvfw_page_prev" id="pvfw-previous-page" onclick="pvfw_prevpage()"><img src="<?php echo plugins_url() . '/' . TNC_PVFW_WEB_DIR . '/'; ?>schemes/light-icons/toolbarButton-pagePrev.svg"  alt=""></button>

		<button style="" class="pvfw_page_next" id="pvfw-next-page" onclick="pvfw_nextpage()"><img src="<?php echo plugins_url() . '/' . TNC_PVFW_WEB_DIR . '/'; ?>schemes/light-icons/toolbarButton-pageNext.svg"  alt=""></button>

		<button style="display: none;" class=" pvfw_page_prev" id="pvfw-flip-previous-page"><img src="<?php echo plugins_url() . '/' . TNC_PVFW_WEB_DIR . '/'; ?>schemes/light-icons/toolbarButton-pagePrev.svg"  alt=""></button>

		<button style="display: none;" class="pvfw_page_next" id="pvfw-flip-next-page"><img src="<?php echo plugins_url() . '/' . TNC_PVFW_WEB_DIR . '/'; ?>schemes/light-icons/toolbarButton-pageNext.svg"  alt=""></button>

...
class Srcset_Helper {
	...
	public function update_image_sizes( $sizes, $size ) {
		...
		if ( is_array( $size ) && $size[0] < $content_width ) {
			return apply_filters( 'wp_smush_image_sizes', $sizes, $size, $content_width, $old_sizes );
		}

		$sizes_value = sprintf( '(max-width: %1$dpx) 100vw, %1$dpx', $content_width );

		return apply_filters( 'wp_smush_image_sizes', $sizes_value, $size, $content_width, $old_sizes );
	}
}